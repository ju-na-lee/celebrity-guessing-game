...
# Global variables
DIR = r'/Users/junaleeiscool/Desktop/Project4/dataset' # Path to image dataset on local device
...
button = ctk.CTkButton(root, width = 300, height = 40, text = "Start Game", font = ctk.CTkFont(size = 15), command = home2game(description, root))
button.pack(pady = 20)
...
def main():
    ...
    features = [] 
    # Celebrities in index form, who's face?
    labels = [] 

    # Runs Haar cascade image training 
    ...
    # Converting features, labels into numpy arrays
    features = np.array(features, dtype = 'object')
    labels = np.array(labels)
    face_recognizer.train(features, labels)

    # Saves training as yaml source file 
    face_recognizer.save('celebrity_game_trained.yml') 

    # Reading in trained yaml file 
